...
void lecturaDeProductos(const char *nom,char ***&productos, int *&stock,
    ...
    cargaarreglos(productos, stock, precios, buffPro, buffStock, buffPre, nDat);
}
...
void cargaarreglos(char ***&productos,int *&stock,double *&precios,char ***buffPro, 
    int *buffStock,double *buffPre,int nDat){
...
void lecturaDePedidos(const char *nom,int *&fechapedidos,
    ...
    while(1){
        ...
        if(ind == NO_ENCONTRADO) agregarfecha(bufffecha,buffcodped,buffcantped,
            fecha,numped,ind,i);
...
void agregarfecha(int *buffecha,char ***buffcodped,int ***buffcantped,int fecha,
    int *numped, int &ind,int &n){
    buffecha[n] = fecha;
...
void reporteDeEnvioDePedidos (const char *nom,char ***productos,int *&stock, 
    ...
    for(int i=0 ; fechaPedidos[i] != 0 ; i++){
        ...
        for(int j=0 ; auxCodPed[j] != nullptr ; j++){
            auxDniCant = aux[j];
            arch << setw(2) << j+1 << ")  " << auxDniCant[0] << setw(12);
