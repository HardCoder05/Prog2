...
void lecturaCursos(const char *nombreArchivo, int* alumno_Codigo, 
    ...
    int**& curso_Alumnos){
    ifstream archEntrada(nombreArchivo, ios::in);
    int nDatos, *buffAlumnos[110];
    double buffCreditos[110];
    char *buffNombre[110];
    ...
    for(int i=0; curso_Alumnos[i] ; i++){
        ...
        for(int j=0; curso_Alumnos[i][2*j] != 0; j++){
            archReporte << right << setw(2) << setfill('0') << (j+1) << 
                setfill(' ') << left << ")  " << setw(7) << curso_Alumnos[i][2*j];
            indiceAlumno = buscarIndiceAlumno(curso_Alumnos[i][2*j],alumno_Codigo);
